...
Primitive::Primitive(std::shared_ptr<BufferElementGL> indices,
    ...
    for (auto iter : attributes){
        ...
        auto gl_accessor = std::make_shared<GLwrap::BufferAccessor>(
            static_cast<GLwrap::ElementStruct>(iter.second->data_size),
            static_cast<GLwrap::ElementType>(iter.second->data_type),
...
void Primitive::draw() const {
    ...
    _vao->drawElements(_mode, static_cast<GLwrap::ElementType>(_indices->data_type), _indices->count, 0);
}
