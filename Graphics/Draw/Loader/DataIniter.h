...
public:
    ...
    virtual std::shared_ptr<Model> newModel() = 0;
    ...
    virtual std::shared_ptr<Buffer> newBuffer(BufferType type,
                                              ...
                                              size_t count, size_t bytes, bool normalized) = 0;
    virtual std::shared_ptr<Buffer> newBuffer(BufferType type,
                                              BufferElemType elem_type, BufferElemStruct elem_struct,
                                              size_t count, size_t bytes, bool normalized) const  = 0;

    virtual std::shared_ptr<Primitive> newPrimitive() = 0;
    virtual std::shared_ptr<Primitive> newPrimitive() const = 0;

    virtual std::shared_ptr<Texture> newTexture(int width, int height, int channels, int bpp) = 0;
